...
function cleanup() {
  ...
  for (const role of [roleA, roleB]) { try { admin(`DROP OWNED BY \"${role}\"; DROP ROLE IF EXISTS \"${role}\";`); } catch { /* no secret or role output */ } }
}
...
function main() {
  ...
  try {
    ...
    const bindingA = sql("SELECT security.current_session_organization_id()", a) === orgA;
    ...
    const ownSubscription = attempt(`INSERT INTO organization_subscriptions (id,\"organizationId\",\"planId\",status,source,\"startsAt\",\"createdAt\",\"updatedAt\") VALUES (${quote(subscriptionA)},${quote(orgA)},${quote(plan)},'ACTIVE','audit',now(),now(),now());`, a);
    const ownEntitlement = attempt(`INSERT INTO organization_entitlements (id,\"organizationId\",\"subscriptionId\",key,status,source,\"startsAt\",\"createdAt\",\"updatedAt\") VALUES (${quote(`r54enta${suffix}`)},${quote(orgA)},${quote(subscriptionA)},'core','ACTIVE','audit',now(),now(),now());`, a);
    ...
    evidence.push({ id: "SP00", result: bindingA ? "PASS" : "FAIL", detail: "temporary tenant role derives organization from session_user mapping" });
    ...
    evidence.push({ id: "SP03", result: ownSubscription && ownEntitlement ? "PASS" : "FAIL", detail: "tenant A creates its own subscription entitlement" });
