...
const checks = [
  ...
  },
  {
    ...
    internalImplementation: status.closed,
    ...externalOrClosed({
      ready: bootstrapAdmin.ready && present("AUTH_SECRET") && process.env.AUTH_SECRET.length >= 32,
      requiredInputs: ["ASAS_BOOTSTRAP_ADMIN_REQUEST_FILE", "AUTH_SECRET"],
      reason: bootstrapAdmin.ready ? "Bootstrap-admin harness is intentionally audit/dry-run only; AUTH_SECRET is still absent or too short." : bootstrapAdmin.reason,
      closedSummary: "Approved bootstrap request and credential-auth secret are present; no production administrator has been created by this preflight.",
    }),
  },
  {
    ...
    internalImplementation: status.closed,
    ...externalOrClosed({
      ready: idpRequest.ready && validHttps("OIDC_ISSUER") && validHttps("OIDC_REDIRECT_URI") && present("OIDC_CLIENT_ID") && present("OIDC_CLIENT_SECRET_FILE"),
      requiredInputs: ["ASAS_IDP_PRODUCTION_REQUEST_FILE", "OIDC_ISSUER", "OIDC_CLIENT_ID", "OIDC_CLIENT_SECRET_FILE", "OIDC_REDIRECT_URI"],
      reason: idpRequest.ready ? "OIDC/SAML contract is installed; issuer/client/secret reference/allow-listed redirect are still required." : idpRequest.reason,
      closedSummary: "IdP request and production OIDC reference inputs are present; this preflight does not contact an issuer.",
    }),
  },
  {
    gate: "mail",
  ...
  },
  {
    ...
    internalImplementation: status.closed,
    ...externalOrClosed({
      ready: schedulerRequest.ready && present("ASAS_SCHEDULER_HEARTBEAT_PATH") && present("ASAS_SCHEDULER_MAX_LAG_SECONDS"),
      requiredInputs: ["ASAS_DOMAIN_SCHEDULER_APPROVAL_FILE", "ASAS_SCHEDULER_HEARTBEAT_PATH", "ASAS_SCHEDULER_MAX_LAG_SECONDS"],
