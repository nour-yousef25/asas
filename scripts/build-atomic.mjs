...
const relativeDistDir = path.posix.join(".build-tmp", `build-${runId}`, ".next");
const nextRoot = path.join(root, relativeDistDir);
const standaloneRoot = path.join(nextRoot, "standalone");

function fail(message) {
  console.error(`ATOMIC_BUILD_FAIL=${message}`);
  process.exit(1);
}
...
if (!path.resolve(nextRoot).startsWith(`${resolvedTemp}${path.sep}`)) fail("TEMP_BUILD_PATH_ESCAPE");

const buildEnv = {
  ...process.env,
  NODE_ENV: "production",
  NEXT_TELEMETRY_DISABLED: "1",
  ASAS_NEXT_DIST_DIR: relativeDistDir,
};
run(process.execPath, ["node_modules/next/dist/bin/next", "build", "--webpack"], buildEnv);
run(process.execPath, ["scripts/package-next-standalone.mjs"], buildEnv);
...
const manifest = {
  contract: "ASAS_ATOMIC_WORKING_TREE_BUILD_V1",
  runId,
  gitHead: text("git", ["rev-parse", "HEAD"]),
  buildId,
  sourceStaticFiles,
  standaloneStaticFiles: targetStaticFiles,
