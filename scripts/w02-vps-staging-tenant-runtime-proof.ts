...
async function main() {
  ...
  try {
    ...
    await record("VPS03", "File provider issues independent checkouts and discard closes each data-plane client", async () => {
      const first = await provider.checkout({ credentialReference: `file://${fixture.principalA}`, principalName: fixture.principalA, organizationId: fixture.organizationA, correlationId: "vps-provider-a1" });
      const second = await provider.checkout({ credentialReference: `file://${fixture.principalA}`, principalName: fixture.principalA, organizationId: fixture.organizationA, correlationId: "vps-provider-a2" });
      try {
    ...
    await record("VPS12", "Target authority, queue, and proof sources contain no Raw GUC identity primitive", async () => {
      ...
      return !/(current_setting|set_config)/.test(source);
    });
  } finally {
...
void main().catch((error) => {
  save({ status: "FAIL_VPS_STAGING_HARNESS", error: error instanceof Error ? "REDACTED_HARNESS_ERROR" : "REDACTED_UNKNOWN", environment: "asasplus_staging_vps", credentialsPersistedInEvidence: false });
