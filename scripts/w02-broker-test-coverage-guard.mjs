...
function record(id, result = 'PASS', overrides = {}) {
  return { id, purpose: 'coverage guard fixture', setup: 'no database', command: 'coverage-gate', inputClass: 'synthetic harness metadata only', expected: 'coverage rule', actual: 'coverage rule', result, databaseIdentity: 'no-database', tenant: 'no-database', organization: null, membershipId: null, leaseId: null, tenantRoleId: null, correlationId: `coverage-${id}`, timestamp: '2026-08-23T00:00:00.000Z', failureReason: null, assertionPassed: result === 'PASS', evidenceRef: `coverage:${id}`, ...overrides };
}

function assertBlocked(name, records, options = {}) {
  const outcome = validateBrokerCoverage(records, options);
  if (outcome.status === 'PASS') throw new Error(`${name} was not blocked`);
...
results.push(assertBlocked('expected actual mismatch', complete.map((item) => item.id === 'B11' ? record('B11', 'PASS', { actual: 'mismatch', assertionPassed: false, failureReason: 'EXPECTED_ACTUAL_MISMATCH' }) : item)));
results.push(assertBlocked('cleanup failure', complete, { cleanup: { ok: false } }));
