...
function record(id, purpose, inputClass, expected, fn, pass) {
  ...
  evidence.push({ id, purpose, setup: 'disposable PostgreSQL audit fixture and broker harness', command: 'broker-harness', inputClass, expected, actual, databaseIdentity: identity, organization: out?.organizationId ?? null, membershipId: out?.membershipId ?? null, leaseId: out?.leaseId ?? null, tenantRoleId: out?.tenantRoleId ?? (out?.sessionUser && out.sessionUser !== 'none' ? out.sessionUser : null), tenant, correlationId, timestamp: new Date().toISOString(), result: result ? 'PASS' : 'FAIL', failureReason: result ? null : reason || actual });
}
...
async function recordAsync(id, purpose, inputClass, expected, fn, pass) {
  ...
  evidence.push({ id, purpose, setup: 'disposable PostgreSQL audit fixture and broker harness', command: 'broker-harness-concurrent', inputClass, expected, actual, databaseIdentity: identity, organization: out?.organizationId ?? null, membershipId: out?.membershipId ?? null, leaseId: out?.leaseId ?? null, tenantRoleId: out?.tenantRoleId ?? null, tenant, correlationId, timestamp: new Date().toISOString(), result: result ? 'PASS' : 'FAIL', failureReason: result ? null : reason || actual });
}
...
async function main() {
  ...
  const coverage = validateBrokerCoverage(evidence);
  const payload = { status: coverage.status === 'PASS' ? 'PASS_BROKER_AUDIT_PROOF' : 'BLOCKED_BROKER_AUDIT_PROOF', startedAt, finishedAt: new Date().toISOString(), databaseAlias: database, postgresVersion: adminSql('SHOW server_version;'), evidence, auditLog, mandatoryIds: coverage.mandatoryIds, executedIds: coverage.executedIds, missingIds: coverage.missingIds, duplicateIds: coverage.duplicateIds, unregisteredIds: coverage.unregisteredIds, invalidIds: coverage.invalidIds, testsWithoutResult: coverage.testsWithoutResult, evidenceMissingFields: coverage.evidenceMissingFields, hardFailures: coverage.hardFailures, coverageFailures: coverage.coverageFailures, securityFailures: coverage.hardFailures, notProven: ['production workload identity separation', 'production credential authority/KMS ownership', 'provider HA/DR and scale', 'production broker deployment'] };
  payload.cleanup = cleanupAuditResources();
  if (!payload.cleanup.ok) {
    payload.status = 'BLOCKED_BROKER_AUDIT_PROOF';
    payload.securityFailures = [...payload.securityFailures, 'AUDIT_CLEANUP_FAILED'];
    payload.coverageFailures = [...payload.coverageFailures, 'AUDIT_CLEANUP_FAILED'];
  }
  writeFileSync(auditFile, `${JSON.stringify(payload, null, 2)}\n`, { mode: 0o600 });
  chmodSync(auditFile, 0o600);
  process.stdout.write(`${JSON.stringify({ status: payload.status, evidenceFile: auditFile, databaseAlias: database, hardFailures: coverage.hardFailures, coverageFailures: coverage.coverageFailures }, null, 2)}\n`);
  process.exitCode = payload.status === 'PASS_BROKER_AUDIT_PROOF' ? 0 : 2;
}

main().catch((error) => {
  const payload = { status: 'FAIL_SETUP_OR_HARNESS', startedAt, finishedAt: new Date().toISOString(), databaseAlias: database, error: String(error.message || error), cleanup: cleanupAuditResources() };
  writeFileSync(auditFile, `${JSON.stringify(payload, null, 2)}\n`, { mode: 0o600 });
