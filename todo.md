...
- [ ] تشغيل regression وhygiene وGit-safe commits/pushes، ثم إصدار W02 COMPLETE أو BLOCKED بتصنيف وحيد صادق.
...
- [x] إجراء Git Safety Gate كامل وتحديد أن النطاقين يتداخلان في docs المشتركة؛ نُفذا بالتسلسل الآمن مع preservation بلا reset/stash.
- [x] تنفيذ `W02-RLS-IDENTITY-CONTRACT-RECONCILIATION` توثيقياً فقط: ADR successor، M6/plan/matrices/registers، identity anchor، lifecycle/support/evidence requirements بلا runtime أو migration.
- [x] تنفيذ `W02-AUDIT-ARTIFACT-HYGIENE-FIX`: metadata-only inventory، revocation/removal آمن، hardening harness، scan redacted، واختبارات cleanup/false-green.
