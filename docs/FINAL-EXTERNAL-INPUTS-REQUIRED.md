...
|---|---|---|---|---|
| Bootstrap | `ASAS_BOOTSTRAP_CONTROL_REQUEST_FILE`: `requestId`,`kind`,`administrator`,`approvedBy`,`approvalReference`,`passwordFile`,`requestedAt`,`expiresAt`. password في `passwordFile` فقط. | request/password `root:root 0600`. | provision/login بشري معتمد ثم evidence redacted؛ لا ينشئه preflight. | احذف password file بعد النجاح، revoke request، وrotate `AUTH_SECRET` مع invalidation للجلسات. |
...
Local VPS Storage وSaaS migrations/RLS/runtime proofs مغلقة الآن. IdP وcertificate activation ليسا شرط SaaS Bootstrap launch.
