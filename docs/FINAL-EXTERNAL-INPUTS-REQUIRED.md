...
|---|---|---|---|---|
| SMTP مؤقت | `ASAS_MAIL_PRODUCTION_REQUEST_FILE` يحدد sender وsandbox recipient المعتمد؛ ثم credential باسم `asas-smtp-sandbox` يحتوي host/port/TLS/username/password/sender/reply-to الاختياري/allow-list/timeout/retry. يستعمل service فقط `ASAS_MAIL_TRANSPORT_CONFIG_CREDENTIAL=asas-smtp-sandbox` و`ASAS_MAIL_DELIVERY_ENABLED=true` بعد approval. | source credential تحت `/opt/asasplus/shared/production-secrets/asas-smtp-sandbox.json` بملكية `root:root` وصلاحية `0600`، ويحمّله systemd كـcredential خاص بالخدمة؛ scope send-only/SMTP submission فقط، لا mailbox أو admin scope. | provider identity/probe ثم sandbox message واحدة إلى recipient المعتمد بعد approval منفصل؛ اختبر TLS/auth/sender/retry/redaction/audit. | rotate credential ثم restart مقيد/re-probe وأبطل السابق. |
