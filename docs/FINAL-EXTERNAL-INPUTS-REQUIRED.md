...
| Scheduler | `ASAS_DOMAIN_SCHEDULER_APPROVAL_FILE` يحوي owner/window/catalogue version/reference، وcatalogue versioned، و`ASAS_SCHEDULER_HEARTBEAT_PATH` و`ASAS_SCHEDULER_MAX_LAG_SECONDS`. | `/opt/asasplus/shared/production-scheduler/`؛ catalogue/approval `root:root 0600`، heartbeat root-owned غير group/world-writable. | parse/deny/dry-run/tenant A-B ثم execution proof مستقل لكل job/owner تحت tenant checkout؛ provider jobs بعد إغلاق بواباتها. | disable service/timer، pause catalogue، احتفظ failure audit، اعزل credentials ثم استبدل catalogue المعتمد. |
