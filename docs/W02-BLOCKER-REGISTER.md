...
| W02-RLS-WAVE-1-RUNTIME-AUTHORITY | Beneficiary RLS runtime identity | Critical | ACTIVE | global Prisma `DATABASE_URL` client is not tenant-bound; Broker audit authority is ephemeral only | Beneficiary RLS Wave 1 and every dependent W02 domain | tenant-bound runtime connection authority and non-owner repository proof |
