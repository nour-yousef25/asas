...
export class BeneficiaryRepository {
  ...
  private async auditDenied(context: TenantContext, action: string, entityId: string) {
    await this.db.auditLog.create({ data: { organizationId: context.organizationId, userId: context.userId, action, entity: "Beneficiary", entityId, details: { actorMembershipId: context.membershipId, decision: "DENY", reasonCode: "TENANT_SCOPE", correlationId: context.correlationId, source: "beneficiary-repository" } } });
  }
  async list(context: TenantContext, input: { skip: number; take: number; search?: string; status?: string }) {
    ...
    const [data, total] = await this.db.$transaction([
      this.db.beneficiary.findMany({ where, skip: input.skip, take: input.take, orderBy: { createdAt: "desc" }, include: { documents: true } }),
      this.db.beneficiary.count({ where }),
    ]);
    return { data, total };
  }

  async getById(context: TenantContext, id: string) {
    const record = await this.db.beneficiary.findFirst({ where: { id, organizationId: context.organizationId }, include: { documents: true } });
  ...
  create(context: TenantContext, input: BeneficiaryInput) {
    return this.db.beneficiary.create({ data: { ...input, organizationId: context.organizationId } });
  }

  async update(context: TenantContext, id: string, input: BeneficiaryUpdate) {
    const record = await this.db.beneficiary.findFirst({ where: { id, organizationId: context.organizationId }, select: { id: true } });
    if (!record) { await this.auditDenied(context, "TENANT_BENEFICIARY_UPDATE_DENIED", id); return null; }
    return this.db.beneficiary.update({ where: { id: record.id }, data: input });
  }

  async deleteOrArchive(context: TenantContext, id: string) {
    const result = await this.db.beneficiary.deleteMany({ where: { id, organizationId: context.organizationId } });
  ...
  async listDocuments(context: TenantContext, beneficiaryId: string) {
    const beneficiary = await this.db.beneficiary.findFirst({ where: { id: beneficiaryId, organizationId: context.organizationId }, select: { documents: true } });
