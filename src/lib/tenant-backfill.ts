...
type DonationOwnershipRow = RootRow & { donorId: string | null; campaignId: string | null; projectId: string | null; donorOrganizationId: string | null; campaignOrganizationId: string | null; projectOrganizationId: string | null };
...
export async function analyzeLegacyTenantBackfill(mappings: ReadonlyArray<LegacyTenantMapping>, client: typeof prisma = prisma): Promise<TenantBackfillReport> {
  ...
  for (const table of LEGACY_TENANT_TABLES) for (const record of await client.$queryRawUnsafe<RootRow[]>(`SELECT "id", "organizationId" FROM "${table}"`)) {
    result.total++; const candidates = byRecord.get(`${table}:${record.id}`) ?? [];
    const ownership = table === "donations" ? donationOwnership.get(record.id) : undefined;
    const parents = ownership ? [[ownership.donorId, ownership.donorOrganizationId], [ownership.campaignId, ownership.campaignOrganizationId], [ownership.projectId, ownership.projectOrganizationId]] as const : [];
    if (parents.some(([id, organizationId]) => id !== null && organizationId === null)) { result.orphan++; blockers.push(`ORPHAN_RECORD:${table}:${record.id}`); continue; }
    const parentOrganizations = new Set(parents.map(([, organizationId]) => organizationId).filter((id): id is string => id !== null));
    if (parentOrganizations.size > 1 || (parentOrganizations.size === 1 && record.organizationId && !parentOrganizations.has(record.organizationId))) { result.conflict++; blockers.push(`CONFLICTING_PARENT_ORGANIZATION:${table}:${record.id}`); continue; }
    ...
    if ((record.organizationId && record.organizationId !== candidates[0].organizationId) || (parentOrganizations.size === 1 && !parentOrganizations.has(candidates[0].organizationId))) { result.conflict++; blockers.push(`CONFLICTING_PARENT_ORGANIZATION:${table}:${record.id}`); continue; }
    result.mapped++; rows.push(candidates[0]);
  }
  for (const [key, entries] of byRecord) if (entries.length !== 1 || !rows.some((row) => `${row.table}:${row.recordId}` === key)) { result.unexpectedNull++; blockers.push(`UNEXPECTED_NULL:${key}`); }
...
export async function applyLegacyTenantBackfill(mappings: ReadonlyArray<LegacyTenantMapping>): Promise<TenantBackfillReport> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    let updated = 0; for (const row of rechecked.rows) updated += await tx.$executeRawUnsafe(`UPDATE "${row.table}" SET "organizationId" = $1 WHERE "id" = $2 AND "organizationId" IS NULL`, row.organizationId, row.recordId);
