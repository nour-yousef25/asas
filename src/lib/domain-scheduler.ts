...
export class DomainSchedulerError extends Error {
  constructor(public readonly code: "CATALOGUE_DENIED" | "JOB_PAUSED" | "JOB_TIMEOUT" | "EXECUTOR_DENIED") { super(`SCHEDULER_${code}`); }
}

export const schedulerJobSchema = z.object({ key: z.string().regex(/^[a-z][a-z0-9_.-]{2,100}$/), owner: z.string().trim().min(3).max(120), version: z.string().regex(/^\d+\.\d+\.\d+$/), tenantMode: z.enum(["TENANT", "SYSTEM"]), intervalSeconds: z.number().int().min(60).max(86_400), timeoutSeconds: z.number().int().min(5).max(900), retryLimit: z.number().int().min(0).max(5), approved: z.boolean(), paused: z.boolean().default(false) });
...
export type SchedulerRunStore = Readonly<{ has(idempotencyKey: string): Promise<boolean>; record(run: SchedulerRun, outcome: "SUCCEEDED" | "FAILED" | "PAUSED"): Promise<void>; heartbeat(at: Date): Promise<void> }>;
...
export class DomainScheduler {
  ...
  async run(input: Readonly<{ jobKey: string; organizationId?: string; scheduledFor: Date; dryRun: boolean }>) {
    ...
    try { await this.executor.execute(run, job.timeoutSeconds * 1000); await this.store.record(run, "SUCCEEDED"); await this.store.heartbeat(this.now()); return { run, outcome: "SUCCEEDED" as const }; }
    catch { await this.store.record(run, "FAILED"); throw new DomainSchedulerError("JOB_TIMEOUT"); }
  }
}
