import { PrismaClient, Role, NewsStatus, MembershipType, MembershipStatus, DonationStatus, ProjectStatus, CampaignStatus, TaskStatus, TaskPriority, EventStatus, SurveyStatus, KPIStatus, KPITargetEntity, EvaluationStatus, EvaluationGoal, EvaluationCompetency, DonorType, DonorStatus, VolunteerStatus, BeneficiaryStatus, Gender, PaymentStatus, InvoiceStatus, OrderStatus, SMSStatus, TransactionType, BudgetStatus, ExpenseStatus, TrialStatus, MeetingStatus, AttendanceStatus, NotificationType } from "@prisma/client";
...
async function main() {
  ...
  // الأدوار - نظّف البيانات الموجودة
  ...
  // الأخبار الحضور مهام استبيانات موظفين
  ...
  await prisma.evaluationGoal.createMany({
    data: [
      { evaluationId: evaluation.id, title: "نشر 24 خبراً نصف سنوي", weight: 30, score: 90, weightedScore: 27 },
      { evaluationId: evaluation.id, title: "تحديث محتوى الموقع", weight: 20, score: 85, weightedScore: 17 },
    ],
  });

  await prisma.evaluationCompetency.createMany({
    data: [
      { evaluationId: evaluation.id, title: "الالتزام بالمواعيد", weight: 15, score: 92, weightedScore: 13.8 },
      { evaluationId: evaluation.id, title: "العمل الجماعي", weight: 10, score: 85, weightedScore: 8.5 },
      { evaluationId: evaluation.id, title: "المهارات التقنية", weight: 25, score: 80, weightedScore: 20 },
    ],
  });
